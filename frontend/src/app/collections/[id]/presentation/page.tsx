  ...
  if (
    ...
    return (
      <div className="min-h-screen bg-black flex flex-col items-center justify-center p-8 text-center font-mono">
        ...
        <h1 className="text-2xl font-black text-white uppercase mb-4 tracking-tighter italic">
          Intelligence Failure
        </h1>
        <p className="text-slate-500 mb-8 max-w-md text-xs uppercase tracking-widest leading-relaxed">
          {error ||
            "This digital archive contains no valid intelligence data or character records."}
        </p>
        <button
          ...
          >
            ...
            <div className="absolute top-0 left-0 right-0 h-10 bg-zinc-900/80 backdrop-blur-md border-b border-zinc-800 flex items-center justify-between px-6 z-20">
              ...
              <div className="flex items-center gap-6">
                <span className="hidden sm:block text-[9px] font-mono text-zinc-600 uppercase tracking-widest truncate max-w-[150px]">
                  Archive: {collection.name}
                </span>
                <button
                  onClick={() => setIsNetworkModalOpen(true)}
                  className="p-2 bg-cyan-500/10 border border-cyan-500/20 text-cyan-500 rounded-lg hover:bg-cyan-500/20 transition-all flex items-center gap-2"
            ...
            <div className="w-full md:w-[400px] bg-zinc-900/10 border-r border-zinc-900 md:p-8 p-4 pt-16 md:pt-16 flex flex-col shrink-0 min-h-0">
              ...
              <div className="mt-6 md:mt-8 grid grid-cols-2 gap-3 md:gap-4">
                <div className="bg-zinc-900/30 p-3 md:p-4 border border-zinc-900 rounded-2xl">
                  <label className="text-[8px] font-mono text-zinc-600 uppercase tracking-widest mb-1 block">
                    Rarity Class
                  </label>
                  <p
                    className={cn(
            ...
            <div className="flex-1 p-6 md:p-12 pt-10 md:pt-20 flex flex-col relative min-h-0">
              <div className="mb-6 md:mb-10 shrink-0">
                <div className="flex items-center gap-4 mb-3 md:mb-4">
                  ...
                  <h2 className="text-[9px] md:text-[10px] font-mono text-cyan-600 uppercase tracking-[0.5em]">
                    {t("collections.id_card")}
                  </h2>
                </div>
                ...
                <div className="inline-flex items-center gap-2 md:gap-3 px-3 md:px-4 py-1.5 bg-cyan-600/5 border border-cyan-500/10 text-cyan-500/80 rounded-full">
                  ...
                  <span className="text-[10px] md:text-xs font-black uppercase tracking-widest italic">
                    {currentCard.name}
                  </span>
                </div>
              </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-10">
                  <div className="space-y-6">
                        ...
                        {(() => {
                          const relations =
                            collection.relations?.filter(
                              (r: any) =>
                                r.sourceCardId === currentCard.id ||
                                r.targetCardId === currentCard.id,
                            ) || [];
                          if (relations.length === 0)
                            return (
                              <p className="text-[10px] text-zinc-600 uppercase italic">
                                No active connections detected in this archive.
                              </p>
                            );

                          return relations.map((rel: any) => {
                            const isSource =
                              rel.sourceCardId === currentCard.id;
                            const otherId = isSource
                              ? rel.targetCardId
                              : rel.sourceCardId;
                            const otherCard = collection.cards.find(
                              (c: any) => c.id === otherId,
                            );
                            const otherIndex = collection.cards.findIndex(
                              (c: any) => c.id === otherId,
                            );
                              ...
                              >
                                <div className="flex items-center gap-3">
                                  <div className="w-8 h-8 rounded bg-zinc-800 overflow-hidden shrink-0">
                                    <img
                                      src={otherCard?.photoUrl}
                                      alt=""
                                      className="w-full h-full object-cover grayscale group-hover/rel:grayscale-0 transition-all"
                                    />
                                  </div>
                                  <div>
                                    <p className="text-[10px] text-white font-black uppercase tracking-tight">
                                      {otherCard?.characterName}
                                    </p>
                                    <p className="text-[8px] text-cyan-500/60 uppercase font-black">
                                      {rel.relationType}
                                    </p>
                                  </div>
                                </div>
                    ...
                    <div>
                      ...
                      <div className="text-zinc-500 font-mono text-xs md:text-sm leading-relaxed border-l border-zinc-800 pl-4 py-1 italic bg-zinc-900/10 rounded-r-lg">
                        {currentCard.details?.description ? (
                          ...
                          <p>
                            Intelligence gathered from archive files suggests
                            this agent holds significant tactical importance.
                            Appearance reported in multiple high-level
                            operations within the {collection.name} timeline.
                          </p>
                        )}
                      </div>
                    </div>

                    {(currentCard.details?.abilities ||
                      currentCard.details?.origin) && (
                      <div className="space-y-6">
                        {currentCard.details?.abilities && (
                          <div>
                            <h3 className="text-[10px] font-mono text-zinc-700 uppercase tracking-widest mb-2 flex items-center gap-2">
                              <Target className="w-3 h-3" />
                              {t("collections.abilities")}
                            </h3>
                            <div className="text-zinc-400 font-mono text-[10px] uppercase font-bold tracking-tight bg-zinc-900/40 p-3 rounded-lg border border-zinc-800/50">
                              <ul className="list-none space-y-1">
                                {currentCard.details.abilities
                                  .split("\n")
                                  .filter((line: string) => line.trim())
              ...
              <div className="mt-8 md:mt-auto flex flex-col sm:flex-row items-center justify-between gap-6 pt-6 md:pt-10 border-t border-zinc-900/50">
                <div className="flex items-center gap-4 md:gap-6">
                  ...
                  >
                    ...
                    <span className="text-[7px] md:text-[8px] font-mono text-zinc-600 uppercase tracking-widest font-bold">
                      Previous
                    </span>
                  </button>
                  ...
                  <div className="flex flex-col items-center gap-2 px-4 md:px-8 min-w-[120px]">
                    <div className="flex items-center gap-1.5">
                      {Array.from({ length: Math.min(totalCards, 8) }).map(
                        (_, i) => (
                          <div
                            key={i}
                            className={cn(
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
            ...
            >
              <div className="p-8 border-b border-zinc-900 flex items-center justify-between shrink-0">
                ...
                <button
                  onClick={() => setIsNetworkModalOpen(false)}
                  className="p-3 bg-zinc-900 hover:bg-zinc-800 rounded-2xl transition-all text-zinc-500"
                >
                  <X className="w-6 h-6" />
                </button>
              </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
                  {collection.cards.map((card: any, idx: number) => {
                    const cardRelations =
                      collection.relations?.filter(
                        (r: any) =>
                          r.sourceCardId === card.id ||
                          r.targetCardId === card.id,
                      ) || [];
                    return (
                      <motion.div
                        key={card.id}
                      ...
                      >
                        ...
                        >
                          ...
                          <h3 className="text-center text-sm font-black text-white uppercase truncate tracking-tighter line-clamp-1 leading-tight">
                            {card.characterName}
                          </h3>
                          <p className="text-center text-[8px] text-zinc-600 uppercase font-bold mb-4 truncate leading-none">
                            {card.name}
                          </p>
                          ...
                          <div className="space-y-1.5 opacity-60 group-hover:opacity-100 transition-opacity">
                            {cardRelations.map((rel: any) => {
                              ...
                              const otherId = isSource
                                ? rel.targetCardId
                                : rel.sourceCardId;
                              const other = collection.cards.find(
                                (c: any) => c.id === otherId,
                              );
                              return (
                                <div
