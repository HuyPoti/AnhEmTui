...
export default function CollectionManagementPage() {
  ...
  const [relationViewMode, setRelationViewMode] = useState<"list" | "tree">(
    "list",
  );
  ...
  const [newCard, setNewCard] = useState({
    ...
    photoUrl: "",
    rarity: "common",
    details: { description: "", abilities: "", origin: "" },
  });
  ...
  const handleAddCard = async (e: React.FormEvent) => {
    ...
    try {
      const token = localStorage.getItem("admin_token");
      const res = await fetch(
        `${API_BASE_URL}/collections/${selectedCollection.id}/cards`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      if (res.ok) {
        ...
        const updatedCollection = await (
          await fetch(`${API_BASE_URL}/collections/${selectedCollection.id}`)
        ).json();
        setCollections((prev) =>
          prev.map((c) =>
  ...
  const handleUpdateCard = async (e: React.FormEvent) => {
    ...
    try {
      const token = localStorage.getItem("admin_token");
      const res = await fetch(
        `${API_BASE_URL}/collections/cards/${editingCard.id}`,
        {
          method: "PATCH",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const handleAddRelation = async (e: React.FormEvent) => {
    ...
    try {
      const token = localStorage.getItem("admin_token");
      const res = await fetch(
        `${API_BASE_URL}/collections/${selectedCollection.id}/relations`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const handleDeleteRelation = async (relationId: string) => {
    ...
    try {
      const token = localStorage.getItem("admin_token");
      const res = await fetch(
        `${API_BASE_URL}/collections/relations/${relationId}`,
        {
          method: "DELETE",
          headers: { Authorization: `Bearer ${token}` },
        },
    ...
    <div className="space-y-10">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-[10px] text-cyan-500 uppercase font-black tracking-widest italic">
            Protocol 04 // Asset Management
          </p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {isLoading
          ? [1, 2, 3].map((i) => (
              <div
                key={i}
                className="h-48 bg-slate-900 rounded-3xl animate-pulse"
              />
            ))
          : collections
              .filter(
                    ...
                    <div className="flex gap-2">
                      {collection.status === "PENDING" && (
                        <>
                          <button
                            onClick={() =>
                              handleStatusUpdate(collection.id, "APPROVED")
                            }
                          ...
                          <button
                            onClick={() =>
                              handleStatusUpdate(collection.id, "REJECTED")
                            }
                      ...
                      <button
                        ...
                        className="p-2 hover:bg-amber-500/20 rounded-lg transition-colors text-slate-400 hover:text-amber-400"
                  ...
                  <div className="space-y-4 relative z-10">
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className="px-2 py-0.5 bg-cyan-500/10 text-cyan-500 rounded text-[8px] font-black uppercase border border-cyan-500/20">
                          {collection.type}
                        </span>
                      ...
                      <p className="text-xs text-slate-500 font-mono italic line-clamp-2">
                        &quot;{collection.description || "No database intel..."}
                        &quot;
                      </p>
                      {collection.author && (
                        <p className="text-[8px] text-slate-600 uppercase font-black mt-2">
                          Submitted by:{" "}
        ...
        {!isLoading && collections.length === 0 && (
          <div className="col-span-full p-20 text-center border-2 border-dashed border-slate-800 rounded-3xl">
            ...
            <p className="text-slate-500 text-xs uppercase font-black tracking-widest">
              {t("admin.collections.no_assets")}
            </p>
          </div>
        )}
      </div>
        ...
        {isCreateModalOpen && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
            ...
            <motion.div
              ...
              className="relative w-full max-w-lg max-h-[90vh] bg-[#0c0c0c] border border-slate-800 rounded-[2.5rem] p-10 overflow-y-auto custom-scrollbar"
            >
              ...
              <form onSubmit={handleCreateCollection} className="space-y-6">
                <div className="space-y-2">
                  <label className="text-[10px] text-slate-500 uppercase font-black tracking-widest ml-1">
                    {t("admin.collections.archive_name")}
                  </label>
                  <input
                    type="text"
                    required
                ...
                <div className="space-y-2">
                  <label className="text-[10px] text-slate-500 uppercase font-black tracking-widest ml-1">
                    {t("admin.collections.intel_desc")}
                  </label>
                  <textarea
                ...
                <div className="space-y-2">
                  ...
                  >
                    <option value="anime" className="bg-slate-950">
                      Anime
                    </option>
                    <option value="movie" className="bg-slate-950">
                      Movies
                    </option>
                    <option value="historical" className="bg-slate-950">
                      Historical
                    </option>
                    <option value="special" className="bg-slate-950">
                      Special Ops
                    </option>
                  </select>
                </div>
        ...
        {isCardModalOpen && selectedCollection && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
            ...
            <motion.div
              ...
              className="relative w-full max-w-5xl h-[90vh] md:h-[85vh] bg-[#0c0c0c] border border-slate-800 rounded-[3rem] overflow-hidden flex flex-col md:flex-row shadow-[0_0_100px_rgba(0,255,255,0.05)]"
            >
              {/* Left Panel: Form */}
              <div className="w-full md:w-[400px] p-10 border-b md:border-b-0 md:border-r border-slate-800 bg-slate-950/30 overflow-y-auto h-[45vh] md:h-full">
                <div className="flex items-center gap-4 mb-8">
                  ...
                  <div>
                    <h2 className="text-xl font-black text-white uppercase tracking-tighter">
                      {editingCard
                        ? "Edit Card"
                        : t("admin.collections.issue_card")}
                    </h2>
                ...
                >
                  ...
                  <div className="space-y-2">
                    <label className="text-[10px] text-slate-500 uppercase font-black tracking-widest ml-1">
                      {t("admin.collections.visual_url")}
                    </label>
                    <div className="relative group">
                      <input
                        ...
                        onChange={(e) =>
                          setNewCard({ ...newCard, photoUrl: e.target.value })
                        }
                  ...
                  <div className="space-y-2">
                    ...
                    <textarea
                      value={newCard.details?.description || ""}
                      onChange={(e) =>
                        setNewCard({
                          ...newCard,
                          details: {
                      ...
                      placeholder="Subject biography and backstory..."
                      rows={2}
                      className="w-full bg-slate-900/50 border border-slate-800 rounded-xl py-4 px-4 text-white text-xs focus:border-cyan-500/50 outline-none transition-all resize-none"
                    />
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <textarea
                      value={newCard.details?.abilities || ""}
                      onChange={(e) =>
                        setNewCard({
                          ...newCard,
                          details: {
                      ...
                      placeholder="Powers, skills, tactical assets... (Use Enter for new lines)"
                      rows={2}
                      className="w-full bg-slate-900/50 border border-slate-800 rounded-xl py-4 px-4 text-white text-xs focus:border-cyan-500/50 outline-none transition-all resize-none"
                    />
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <textarea
                      ...
                      placeholder="Home world, organization, squad... (Use Enter for new lines)"
                      rows={2}
                      className="w-full bg-slate-900/50 border border-slate-800 rounded-xl py-4 px-4 text-white text-xs focus:border-cyan-500/50 outline-none transition-all resize-none"
                    />
                  </div>
              ...
              <div className="flex-1 p-6 md:p-10 flex flex-col min-h-0 h-[45vh] md:h-full">
                <div className="flex items-center justify-between mb-8 shrink-0">
                  <div className="flex items-center gap-4">
                    <Info className="w-4 h-4 text-slate-600" />
                    <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">
                      Archive contains {selectedCollection.cards?.length || 0}{" "}
                      identifying assets
                    </p>
                  </div>
                  <button
                ...
                <div className="flex-1 overflow-y-auto grid grid-cols-4 lg:grid-cols-4 pr-2 gap-2 custom-scrollbar items-start content-start">
                  {selectedCollection.cards?.map((card: any) => (
                    <motion.div
                      key={card.id}
                      initial={{ opacity: 0, y: 10 }}
                      animate={{ opacity: 1, y: 0 }}
                      className="group w-full aspect-[3/4] bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden relative shadow-lg  cursor-pointer"
                    >
                      <img
                        src={card.photoUrl}
                        alt={card.name}
                        className="absolute w-full h-full object-cover opacity-60 group-hover:scale-110 group-hover:opacity-100 transition-all duration-500"
                      />
                      {/* <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/20 to-transparent" /> */}

                      <div className="absolute left-0 right-0 top-8 mx-auto space-y-2 z-10 flex flex-col items-center justify-center">
                        <div className="flex items-center">
                          <span
                            className={cn(
                              "px-2 py-0.5 rounded text-[7px] font-black uppercase border leading-none",
                        ...
                        <div className="space-y-0.5">
                          <h4 className="text-[11px] text-white font-black uppercase tracking-tight line-clamp-1 leading-tight">
                            {card.characterName}
                          </h4>
                          <p className="text-[7px] text-slate-400 font-bold uppercase tracking-widest leading-none opacity-60">
                            {card.name}
                          </p>
                        </div>
                        <div>
                          <button
                            ...
                            className="p-1.5 bg-slate-900/90 backdrop-blur-md text-cyan-400 border border-cyan-500/30 rounded-lg transition-all shadow-xl hover:bg-cyan-500 hover:text-white"
                          >
                          ...
                          <button
                            ...
                            className="p-1.5 bg-slate-900/90 backdrop-blur-md text-red-400 border border-red-500/30 rounded-lg transition-all shadow-xl hover:bg-red-500 hover:text-white"
                          >
                  ...
                  {(!selectedCollection.cards ||
                    ...
                    <div className="col-span-full h-full flex flex-col items-center justify-center border-2 border-dashed border-slate-800 rounded-3xl opacity-30 p-10">
                      ...
                      <p className="text-xs font-black uppercase tracking-widest text-center">
                        No visual profiles issued for this collection
                      </p>
                    </div>
                  )}
                </div>
        ...
        {isRelationModalOpen && selectedCollection && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
            ...
            <motion.div
              ...
              exit={{ scale: 0.95, opacity: 0 }}
              className={cn(
                "relative w-full h-[90vh] md:h-[85vh] bg-[#0c0c0c] border border-slate-800 rounded-[3rem] overflow-hidden flex flex-col md:flex-row shadow-[0_0_100px_rgba(245,158,11,0.05)] transition-all duration-500",
                relationViewMode === "tree" ? "max-w-[95vw]" : "max-w-5xl",
              )}
            >
              {/* Left Panel: Create Relation */}
              {relationViewMode === "list" && (
                <div className="w-full md:w-[400px] p-10 border-b md:border-b-0 md:border-r border-slate-800 bg-slate-950/30 overflow-y-auto shrink-0 h-[45vh] md:h-full">
                  <div className="flex items-center gap-4 mb-8">
                  ...
                  <form onSubmit={handleAddRelation} className="space-y-6">
                    ...
                    <button
                      disabled={
                        isSubmitting ||
                        !newRelation.sourceCardId ||
                        !newRelation.targetCardId ||
                        newRelation.sourceCardId === newRelation.targetCardId
                      }
              ...
              <div className="flex-1 p-6 md:p-10 flex flex-col min-h-0 h-[45vh] md:h-full">
                <div className="flex items-center justify-between mb-8 shrink-0">
                  <div className="flex items-center gap-4">
                    ...
                    <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">
                      Archive contains{" "}
                      {selectedCollection.relations?.length || 0} established
                      connections
                    </p>
                  </div>
                ...
                {relationViewMode === "tree" ? (
                  ...
                  <div className="flex-1 overflow-y-auto space-y-4 pr-2 custom-scrollbar">
                    {selectedCollection.relations?.map((rel: any) => {
                      const source = selectedCollection.cards.find(
                        (c: any) => c.id === rel.sourceCardId,
                      );
                      const target = selectedCollection.cards.find(
                        (c: any) => c.id === rel.targetCardId,
                      );
                      return (
                        <div
                          key={rel.id}
                          className="bg-slate-900/50 border border-slate-800 p-4 rounded-2xl flex items-center justify-between group"
                        >
                          <div className="flex items-center gap-6">
                            <div className="text-right">
                              <p className="text-[10px] text-white font-black uppercase tracking-tighter">
                                {source?.characterName || "Unknown"}
                              </p>
                          ...
                          <button
                            ...
                            className="p-2 bg-red-500/10 text-red-500 border border-red-500/20 rounded-lg transition-all"
                          >
                            <Trash2 className="w-4 h-4" />
                          </button>
                        </div>
                    ...
                    {(!selectedCollection.relations ||
                      selectedCollection.relations.length === 0) && (
                      <div className="h-full flex flex-col items-center justify-center border-2 border-dashed border-slate-800 rounded-3xl opacity-30 p-10">
                        ...
                        <p className="text-xs font-black uppercase tracking-widest text-center">
                          No character connections established yet
                        </p>
                      </div>
                    )}
                  </div>
