...
export default function AdminDashboardPage() {
  ...
  const cards = [
    ...
    {
      label: t("admin.dashboard.stats.trees"),
      value: stats?.treeCount || 0,
      icon: TreeDeciduous,
        ...
        {cards.map((card, i) => (
          ...
          >
            ...
            <div className="flex items-center justify-between mb-4 relative z-10">
              <div
                className={cn(
                  "w-12 h-12 rounded-xl flex items-center justify-center border border-current shadow-2xl",
                  card.color,
                  card.bg,
                )}
              >
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
        <div className="xl:col-span-2 space-y-6">
          <div className="flex items-center justify-between">
            ...
            <button className="text-[10px] text-cyan-500 uppercase font-black hover:underline">
          ...
          <div className="bg-slate-900/40 border border-slate-800 rounded-3xl overflow-hidden divide-y divide-slate-800/50">
            {activity.length === 0 ? (
              ...
              activity.map((item) => (
                <div
                  key={item.id}
                  className="p-4 flex items-center justify-between hover:bg-slate-800/20 transition-colors group"
                >
                  ...
                  <div className="flex items-center gap-2">
                    <span className="px-2 py-0.5 bg-slate-800 text-slate-400 text-[8px] font-black rounded border border-slate-700 uppercase">
                      System
                    </span>
